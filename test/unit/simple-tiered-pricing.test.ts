...
import {
  ...
  getModelPricing,
} from "../../src/token-cost-calculations.js";
...
describe("Simple Tiered Pricing Support", () => {
  test("should handle Gemini 2.5 Pro 200k tier pricing", () => {
    ...
    expect(aboveThreshold.actualCost.inputCost).toBeGreaterThan(
      belowThreshold.actualCost.inputCost,
    );
    ...
    expect(belowThreshold.actualCost.inputCost).toBeCloseTo(
      expectedBelowInput,
      6,
    );
    expect(aboveThreshold.actualCost.inputCost).toBeCloseTo(
      expectedAboveInput,
      6,
    );
    expect(belowThreshold.actualCost.outputCost).toBeCloseTo(
      expectedOutputBelow,
      6,
    );
    expect(aboveThreshold.actualCost.outputCost).toBeCloseTo(
      expectedOutputAbove,
      6,
    );
  });
  ...
  test("should calculate cache savings correctly with tiered pricing", () => {
    ...
    expect(analysis.uncachedCost.inputCost).toBeGreaterThan(
      analysis.actualCost.inputCost,
    );
  ...
  test("should extract tier attributes from model pricing", () => {
    ...
    expect(pricing.input_cost_per_token_above_200k_tokens).toBeGreaterThan(
      pricing.input_cost_per_token,
    );
    expect(pricing.output_cost_per_token_above_200k_tokens).toBeGreaterThan(
