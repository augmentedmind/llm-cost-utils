 ...
 */
export function extractTokenUsageFromResponseBody(
  responseBody: any,
): TokenUsageWithModel {
  ...
  if (responseBody?.message?.usage) {
    ...
    completionTokens = Math.max(
      0,
      aiSdkCompletionTokens - aiSdkReasoningTokens,
    );
    ...
    if (responseBody.model?.startsWith("mistral-")) {
      ...
      if (
        ...
        responseBody.usage.completion_tokens_details?.reasoningTokens !==
        undefined
      ) {
        reasoningTokens =
      ...
      if (
        ...
        promptCacheMissTokens = Math.max(
          0,
          totalPromptTokens - promptCacheHitTokens,
        );
      } else {
 ...
 */
export function extractTokenUsageFromStreamingResponseBody(
  responseText: string,
): TokenUsageWithModel {
  // Throw error for null or undefined response
  if (!responseText) {
    throw new TokenUsageExtractionError(
      "Token usage extraction failed: response text is null or undefined",
    );
  }

  // Check for error events in the stream
  if (
    responseText.includes("event: error") ||
    responseText.includes('"type":"error"')
  ) {
    throw new TokenUsageExtractionError(
      "Token usage extraction failed: error event detected in stream",
    );
  }
      ...
      if (dataLine) {
        ...
        try {
          ...
          if (data.message?.usage || data.usage || data.usageMetadata) {
            ...
            tokenUsage.promptCacheMissTokens = Math.max(
              tokenUsage.promptCacheMissTokens,
              eventTokenUsage.promptCacheMissTokens,
            );
            tokenUsage.promptCacheHitTokens = Math.max(
              tokenUsage.promptCacheHitTokens,
              eventTokenUsage.promptCacheHitTokens,
            );
            tokenUsage.reasoningTokens = Math.max(
              tokenUsage.reasoningTokens,
              eventTokenUsage.reasoningTokens,
            );
            tokenUsage.completionTokens = Math.max(
              tokenUsage.completionTokens,
              eventTokenUsage.completionTokens,
            );
            tokenUsage.totalOutputTokens = Math.max(
              tokenUsage.totalOutputTokens,
              eventTokenUsage.totalOutputTokens,
            );
            tokenUsage.totalInputTokens = Math.max(
              tokenUsage.totalInputTokens,
              eventTokenUsage.totalInputTokens,
            );
            tokenUsage.promptCacheWriteTokens = Math.max(
              tokenUsage.promptCacheWriteTokens,
              eventTokenUsage.promptCacheWriteTokens,
            );
  ...
  if (tokenUsage.totalInputTokens === 0 && tokenUsage.totalOutputTokens === 0) {
    ...
    throw new TokenUsageExtractionError(
      "Token usage extraction failed: no token usage information found in streaming response",
    );
  }
